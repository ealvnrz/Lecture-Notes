...
\begin{figure}
\centering
\includegraphics[width=2.60417in,height=\textheight]{../resources/functional_relation.png}
...
\begin{figure}
\centering
\includegraphics[width=2.60417in,height=\textheight]{../resources/statistical_relation.png}
\caption{Relación Funcional}
\end{figure}
...
\begin{figure}
\centering
\includegraphics[width=5in,height=\textheight]{../resources/regression_steps.png}
...
\begin{figure}
\centering
\includegraphics[width=0.45\textwidth,height=\textheight]{../resources/regression_1.png}
...
\begin{figure}
\centering
\includegraphics[width=0.45\textwidth,height=\textheight]{../resources/regression_2.png}
\caption{Criterio Q para un ajuste de regresión}
\end{figure}
...
\KeywordTok{data}\NormalTok{(UScereal)}
\KeywordTok{ggplot}\NormalTok{(UScereal,}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x=}\NormalTok{fibre,}\DataTypeTok{y=}\NormalTok{calories)) }\OperatorTok{+}\StringTok{ }\KeywordTok{geom_point}\NormalTok{() }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_smooth}\NormalTok{(}\DataTypeTok{method=}\NormalTok{lm,}\DataTypeTok{se=}\OtherTok{FALSE}\NormalTok{,}\DataTypeTok{color=}\StringTok{"red"}\NormalTok{) }
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{model <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(calories}\OperatorTok{~}\NormalTok{fibre,}\DataTypeTok{data=}\NormalTok{UScereal)}
...
\hypertarget{diagnuxf3stico-y-medidas-correctivas}{%
\subsection{Diagnóstico y medidas
correctivas}\label{diagnuxf3stico-y-medidas-correctivas}}

\begin{frame}{Diagnóstico y medidas correctivas}
...
resudio observado \(e_i\) deben reflejar las propiedades que se
...
\emph{studentizados}. Sim embargo, la desviación estándar de \(e_i\) es
...
Graficar los residuos vs la variable predictora or vs los valores
...
\item
  ...
  un grafico de probabilidad normal de los residuos. En donde se grafica
...
cualitativa que representa el estado de discapicidad. Podemos agregar
